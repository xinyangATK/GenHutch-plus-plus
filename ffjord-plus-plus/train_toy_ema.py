...
parser.add_argument("--divergence_fn", type=str, default="hutchplusplus", choices=["brute_force", "approximate", "approximate_m", "hutchplusplus", "hutchplusplus_m", "na_hutchplusplus"])
...
parser.add_argument("--step_size", type=float, default=0.01, help="Optional fixed step size.")
...
if __name__ == '__main__':
    ...
    # load EMA
    if args.ema_decay > 0.:
        model_ema = load_ema(model, decay=args.ema_decay)
    else:
        model_ema = None
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.val_freq == 0 or itr == args.niters:
            with torch.no_grad():
                ...
                if args.ema_decay > 0.:
                    with ema_scope(args, model_ema, model, 'Test with EMA'):
                        test_loss = compute_loss(args, model, batch_size=args.test_batch_size)
                        test_nfe = count_nfe(model)
                        log_message = '[TEST] Iter {:04d} | Test Loss {:.6f} | NFE {:.0f}'.format(itr, test_loss, test_nfe)
                        logger.info(log_message)
                ...
                torch.save({
                    ...
                    'ema_state_dict': model_ema.state_dict() if args.ema_decay > 0. else None,
        ...
        if itr % args.viz_freq == 0:
            ...
            with torch.no_grad():
                ...
                # Evaluation Process
                with ema_scope(args, model_ema=model_ema, model=model, context='Evaluation'):
                    sample_fn, density_fn = get_transforms(model)

                    plt.figure(figsize=(9, 3))
                    _ = visualize_transform(
                        p_samples, torch.randn, standard_normal_logprob, transform=sample_fn, inverse_transform=density_fn,
                        samples=True, npts=npts, device=device,
                    )
